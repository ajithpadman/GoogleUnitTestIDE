...
namespace GnuCoverageAnalyser
{
    public class GenerateCoverageReport
    {
        ...
        public GenerateCoverageReport(GnuCoverageAnalyser analyser)
        {
            m_analyser = analyser;
            m_summary = analyser.CoverageSummary;
            m_DetailedCoverage = analyser.DetailedCoverage;

            m_coverageReportPath = analyser.ReportPath;
        }
        ...
        public void Generate()
        {
            ...
            if (Directory.Exists(m_coverageReportPath))
            {
                ...
                using (HtmlTextWriter writer = new HtmlTextWriter(stringWriter))
                {
                    ...
                    writer.AddStyleAttribute(HtmlTextWriterStyle.TextAlign, "left");
                    writer.RenderBeginTag(HtmlTextWriterTag.H3);
                    writer.WriteLine("Project:" + m_analyser.Owner.ProjectData.ProjectName);
                    writer.RenderEndTag();//H3
                    ...
                    {
                        if (m_analyser.Owner.SelectedFiles.Contains(summary.m_fileName))
                        {
                            writer.RenderBeginTag(HtmlTextWriterTag.Tr);
                            writer.RenderBeginTag(HtmlTextWriterTag.Td);
